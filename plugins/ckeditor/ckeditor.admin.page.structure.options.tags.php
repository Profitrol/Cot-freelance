...
Code=ckeditor
Part=admin
File=ckeditor.admin.page.structure.options.tags
Hooks=admin.page.structure.options.tags
Tags=
Order=10
[END_SED_EXTPLUGIN]
==================== */
...
if ($sed_groups[$usr['maingrp']]['pfs_maxtotal']>0 && $sed_groups[$usr['maingrp']]['pfs_maxfile']>0 && sed_auth('pfs', 'a', 'R')){ 
	if (!$cfg['disable_pfs']){
		$pfsUser = "<a href=\"javascript:popup('ckeditor&userid=".$usr['id']."&c1=newpage&c2=rdesc', 754, 512);\">".$L['Mypfs']."</a>"; 
	}
}

if ($sed_groups[$usr['maingrp']]['pfs_maxtotal']>0 && $sed_groups[$usr['maingrp']]['pfs_maxfile']>0 && sed_auth('pfs', 'a', 'R')){ 
		$pfsSite = (sed_auth('pfs', 'a', 'A')) ?  "<a href=\"javascript:popup('ckeditor&userid=0&c1=newpage&c2=rdesc', 754, 512);\">".$L['SFS']."</a>" : ""; 
}

$resize = ($cfg['plugin']['ckeditor']['resize_enabled']) ? 'true' : 'false';
$editt = "
<script type=\"text/javascript\">
	CKEDITOR.replace( 'rdesc',{
