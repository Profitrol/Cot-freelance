...
$rel = ($cfg['plugin']['ckeditor']['useThumbnailviewer']) ? 'rel="thumbnail"' : 'target="_blank"';
...
elseif ($a=='delete')
{
	...
	if ($row = sed_sql_fetcharray($sql))
	{
		...
		header("Location: " . SED_ABSOLUTE_URL . sed_url('plug',"o=ckeditor&".$more, '', true));
		exit;
	}
	else
	{ sed_die(); }
...
elseif ($a=='newfolder')
{
	...
	(".(int)$userid.",
		...
		".(int)$nisgallery.",
		0)");
	header("Location: " . SED_ABSOLUTE_URL . sed_url('plug',"o=ckeditor&".$more, '', true)); // by Alex
	exit;
}
...
{	
	...
	while ($row1 = sed_sql_fetcharray($sql1l))
	{
		...
		$list_folders .= "<tr><td>[<a href=\"".sed_url('plug','o=ckeditor&a=deletefolder&'.sed_xg().'&f='.$pff_id.'&'.$more)."\">x</a>]</td>";
		$list_folders .= "<td><a href=\"".sed_url('plug','o=ckeditor&m=editfolder&f='.$pff_id.'&'.$more)."\">".$L['Edit']."</a></td>";
		...
		$list_folders .= "<td><a href=\"".sed_url('plug','o=ckeditor&f='.$pff_id.'&'.$more)."\">".$icon_f."</a></td>";
		$list_folders .= "<td><a href=\"".sed_url('plug','o=ckeditor&f='.$pff_id.'&'.$more)."\">".$pff_title."</a></td>";
...
while ($row = sed_sql_fetcharray($sqll))
{
	...
	$add_file = ($standalone) ? "<a href=\"javascript:addfile('".$pfs_file."','".$c2."', '".htmlspecialchars($pfs_desc)."')\"><img src=\"skins/".$skin."/img/system/icon-pastefile.gif\" title=\"Add as file url\" /></a>" : '';
	...
	if (in_array($pfs_extension, $gd_supported) && $cfg['th_amode']!='Disabled')
	{
		...
		if ($standalone)
		{
			$add_thumbnail .= "<a href=\"javascript:addthumb('".$pfs_file."','".$c2."', '".htmlspecialchars($pfs_desc)."')\"><img src=\"skins/".$skin."/img/system/icon-pastethumb.gif\" title=\"Add thumbnail\" /></a>";
			$add_image = "<a href=\"javascript:addpix('".$pfs_file."','".$c2."', '".htmlspecialchars($pfs_desc)."')\"><img src=\"skins/".$skin."/img/system/icon-pasteimage.gif\" title=\"Add image\" /></a>";
		}
		if ($o=='thumbs')
		{ $pfs_icon = "<a href=\"".$pfs_fullfile."\"><img src=\"".$cfg['th_dir_user'].$pfs_file."\" title=\"".$pfs_file."\"></a>"; }
	}

	$list_files .= "<tr><td>[<a href=\"".sed_url('plug','o=ckeditor&a=delete&'.sed_xg().'&id='.$pfs_id.'&'.$more.'&o='.$o)."\">x</a>]</td>";
	$list_files .= "<td><a href=\"".sed_url('plug','o=ckeditor&m=edit&id='.$pfs_id.'&'.$more)."\">".$L['Edit']."</a></td>";
	$list_files .= "<td>".$pfs_icon."</td>";
	$list_files .= "<td><a href=\"".$pfs_fullfile."\" ";
	if ($filedesc[$pfs_extension] == 'Picture') $list_files .= $rel;
	$list_files .= " >".$pfs_file."</a></td>";
...
$disp_upload .= "<form enctype=\"multipart/form-data\" action=\"".sed_url('plug','o=ckeditor&a=upload'.'&'.$more)."\" method=\"post\">"; // by Alex
...
if ($f==0 && $usr['auth_write'])
{
	...
	$disp_newfolder .= "<form id=\"newfolder\" action=\"".sed_url('plug','o=ckeditor&a=newfolder'.'&'.$more)."\" method=\"post\">";  // by Alex
...
if ($standalone)
{
	...
	{
		$path = $cfg['pfs_dir_user'];
		if (strncmp($path, "./", 2) == 0) {
			$path = substr($path, 2);
		}
		//$path = $cfg['mainurl']."/".$path;
		$thPath = $cfg['th_dir_user'];
		if (strncmp($thPath, "./", 2) == 0) {
			$thPath = substr($thPath, 2);
		}
		//$thPath = $cfg['mainurl']."/".$thPath;
		
		$addthumb = "'<a href=\"$path'+gfile+'\" $rel title=\"'+title+'\"><img src=\"$thPath'+gfile+'\" border=\"0\" alt=\"'+title+'\" /></a>'";
		$addpix = "'<img src=\"$path'+gfile+'\" alt=\"'+title+'\" />'";
		$addfile = "'<a href=\"$path'+gfile+'\" title=\"'+title+'\" >'+gfile+'</a>'";
	}
	$winclose = $cfg['pfs_winclose'] ? "\nwindow.close();" : '';
	$pfs_header1 = $cfg['doctype']."<html><head>
<title>".$L['pfs_title']." - ".$cfg['maintitle']."</title>".sed_htmlmetas().sed_javascript()."
...
function addthumb(gfile,c2,title) {
	var element = window.opener.CKEDITOR.dom.element.createFromHtml( $addthumb );
	eval('window.opener.CKEDITOR.instances.' + c2 + '.insertElement( element );');
	$winclose
}
function addpix(gfile,c2,title) {
	var element = window.opener.CKEDITOR.dom.element.createFromHtml( $addpix );
	eval('window.opener.CKEDITOR.instances.' + c2 + '.insertElement( element );');
	$winclose
}
function addfile(gfile,c2,title) {
	var element = window.opener.CKEDITOR.dom.element.createFromHtml( $addfile );
	eval('window.opener.CKEDITOR.instances.' + c2 + '.insertElement( element );');
	$winclose
}
...
";
if ($cfg['plugin']['ckeditor']['useThumbnailviewer']){
	$pfs_header1 .= "<script type=\"text/javascript\" src=\"{$cfg['plugins_dir']}/thumbnailviewer/inc/thumbnailviewer.js\"></script>
<link type=\"text/css\" href=\"{$cfg['plugins_dir']}/thumbnailviewer/inc/thumbnailviewer.css\" rel=\"stylesheet\" />";
}
	sed_sendheaders();
