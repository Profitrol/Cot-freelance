...
$adminpath[] = array ("admin.php?m=page&s=add", 'Добавление страницы');
$adminhelp = $L['adm_help_page'];

list($usr['auth_read'], $usr['auth_write'], $usr['isadmin']) = sed_auth('page', 'any');

/* === Hook === */
$extp = sed_getextplugins('admin.page.add.first');
if (is_array($extp))
...
if ($a=='add')
{
	...
	/* === Hook === */
	$extp = sed_getextplugins('admin.page.add.add.first');
	if (is_array($extp))
	{ foreach($extp as $k => $pl) { include_once($cfg['plugins_dir'].'/'.$pl['pl_code'].'/'.$pl['pl_file'].'.php'); } }
	/* ===== */
	...
	if (empty($error_string))
	{
		...
		/* === Hook === */
		$extp = sed_getextplugins('admin.page.add.add.query');
		if (is_array($extp))
		{ foreach($extp as $k => $pl) { include_once($cfg['plugins_dir'].'/'.$pl['pl_code'].'/'.$pl['pl_file'].'.php'); } }
		/* ===== */
...
$ssql.="page_title,
  	...
  	$ssql.="'".sed_sql_prep($newpagetitle)."',
		...
		$id = sed_sql_insertid();
		$r_url = sed_url('admin', "m=pages&c=".$newpagecat, '', true);

		/* === Hook === */
		$extp = sed_getextplugins('admin.page.add.add.done');
		if (is_array($extp))
		{ foreach($extp as $k => $pl) { include_once($cfg['plugins_dir'].'/'.$pl['pl_code'].'/'.$pl['pl_file'].'.php'); } }
		/* ===== */
...
$newpage_form_begin = sed_selectbox_date($sys['now_offset']+$usr['timezone']*3600, 'short', '_beg');
...
/* === Hook === */
$extp = sed_getextplugins('admin.page.add.main');
if (is_array($extp))
{ foreach($extp as $k => $pl) { include_once($cfg['plugins_dir'].'/'.$pl['pl_code'].'/'.$pl['pl_file'].'.php'); } }
/* ===== */

$t = new XTemplate(sed_skinfile('admin.page.add.inc', false, true));
...
$pageadd_array = array(
	...
	"PAGEADD_FORM_SEND" => sed_url('admin', "m=page&s=add&a=add"),
	...
	"PAGEADD_FORM_DESC" => "<textarea class=\"text\" name=\"newpagedesc\" rows=\"5\" style=\"width:99%;\">".htmlspecialchars($newpagedesc)."</textarea>",
...
/* === Hook === */
$extp = sed_getextplugins('admin.page.add.tags');
if (is_array($extp))
{ foreach($extp as $k => $pl) { include_once($cfg['plugins_dir'].'/'.$pl['pl_code'].'/'.$pl['pl_file'].'.php'); } }
/* ===== */
