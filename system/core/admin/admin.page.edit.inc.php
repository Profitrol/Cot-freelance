...
$adminpath[] = array ("admin.php?m=page&s=edit&id=".$id, 'Редактирование страницы');
...
if ($a=='update')
{
	...
	/* === Hook === */
	$extp = sed_getextplugins('admin.page.edit.update.first');
	if (is_array($extp))
	...
	if (empty($error_string) || $rpagedelete)
	{
		if ($rpagedelete)
		{
			...
			if ($row = sed_sql_fetchassoc($sql))
			{
				...
				/* === Hook === */
				$extp = sed_getextplugins('admin.page.edit.delete.done');
				if (is_array($extp))
				{ foreach($extp as $k => $pl) { include_once($cfg['plugins_dir'].'/'.$pl['pl_code'].'/'.$pl['pl_file'].'.php'); } }
				/* ===== */
				header("Location: " . SED_ABSOLUTE_URL . sed_url('admin', "m=pages", '', true));
				exit;
			}
		}
		else
		{
			...
			/* === Hook === */
			$extp = sed_getextplugins('admin.page.edit.update.done');
			if (is_array($extp))
			{ foreach($extp as $k => $pl) { include_once($cfg['plugins_dir'].'/'.$pl['pl_code'].'/'.$pl['pl_file'].'.php'); } }
			/* ===== */

			sed_log("Edited page #".$id,'adm');
			header("Location: " . SED_ABSOLUTE_URL . sed_url('admin', "m=pages&c=".$rpagecat, '', true));
			exit;
		}
	}
}
...
$pag['page_begin'] = sed_selectbox_date($pag['page_begin'] + $usr['timezone'] * 3600, 'short', '_beg');
...
/* === Hook === */
$extp = sed_getextplugins('admin.page.edit.first');
if (is_array($extp))
{ foreach($extp as $k => $pl) { include_once($cfg['plugins_dir'].'/'.$pl['pl_code'].'/'.$pl['pl_file'].'.php'); } }
/* ===== */
...
/* === Hook === */
$extp = sed_getextplugins('admin.page.edit.main');
if (is_array($extp))
{ foreach($extp as $k => $pl) { include_once($cfg['plugins_dir'].'/'.$pl['pl_code'].'/'.$pl['pl_file'].'.php'); } }
/* ===== */

$t = new XTemplate(sed_skinfile('admin.page.edit.inc', false, true));
...
$pageedit_array = array(
	...
	"PAGEEDIT_FORM_SEND" => sed_url('admin', "m=page&s=edit&a=update&id=".$pag['page_id']."&r=".$r),
	...
	"PAGEEDIT_FORM_DESC" => "<textarea class=\"text\" name=\"rpagedesc\" rows=\"5\" style=\"width:99%;\">".htmlspecialchars($pag['page_desc'])."</textarea>",
...
/* === Hook === */
$extp = sed_getextplugins('admin.page.edit.tags');
if (is_array($extp))
{ foreach($extp as $k => $pl) { include_once($cfg['plugins_dir'].'/'.$pl['pl_code'].'/'.$pl['pl_file'].'.php'); } }
/* ===== */
